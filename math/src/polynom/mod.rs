...
/// Synthetic division but also returns the remainder
/// Specifically, divides polynomial `p` by polynomial (x^`a` - `b`) using
pub fn syn_div_with_rem<E>(dividend: &[E], a: usize, b: E) -> (Vec<E>, Vec<E>)
where
    E: FieldElement,
{
    assert!(a != 0, "divisor degree cannot be zero");
    assert!(b != E::ZERO, "constant cannot be zero, just rotate");

    if dividend.len() <= a {
        // divisor has larger degree than dividend
        return (vec![E::ZERO; dividend.len()], dividend.to_vec());
    }

    let mut p = dividend.to_vec();

    let degree_offset = p.len() - a;
    // if `b` is 1 or -1, no need to multiply by `b` in every iteration of the loop
    if b == E::ONE {
        for i in (0..degree_offset).rev() {
            p[i] = p[i] + p[i + a];
        }
    ...
    // remainder is stored in 0..a elements
    // quotient is stored in a.. elements
    let mut quotient = p[a..].to_vec();
    let mut remainder = p[0..a].to_vec();

    quotient.resize(dividend.len(), E::ZERO);
    remainder.resize(dividend.len(), E::ZERO);
