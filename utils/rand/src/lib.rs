...
mod internal {
    ...
    /// sample a random vector of field elements within a given range
    /// specify the range as m number of bits (must be a multiple of 8)
    pub fn rand_vector_range<R: Randomizable>(n: usize, m: usize) -> Vec<R> {
        debug_assert!(
            m % 8 == 0 && m >= 8 && m <= 256,
            "m must be a multiple of 8, larger than 8 and smaller than 256"
        );
        let m_bytes = m / 8;
        let mut result = Vec::with_capacity(n);
        let seed = rand::thread_rng().gen::<[u8; 32]>();
        let mut g = StdRng::from_seed(seed);
        for _ in 0..1000 * n {
            let mut bytes = g.gen::<[u8; 32]>();
            for i in m_bytes..bytes.len() {
                bytes[i] = 0;
            }
            if let Some(element) = R::from_random_bytes(&bytes[..R::VALUE_SIZE]) {
                result.push(element);
